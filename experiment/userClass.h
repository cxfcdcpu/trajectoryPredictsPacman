...
class User
{
  ...
  public:
    ...
    void trainingExample(vector<int> anchors);
    string randomNetwork(short nodeNum, short anchorNum, short radioRange);
    void getTrainInfo(short hv[][anchorSize], short hopLength);
    vector<hyperTrial> findTrainHyperTrial(short hv[][anchorSize]);
    vector<hyperTrial> trainHyperHelper(short hv[][anchorSize],int  ah,int  h3);
};
...
void User:: trainingExample(vector<int> anchors)
{
  
  anchor = anchors.size()/2;
  ...
  {
    xList[i]=anchors[counter++];
    yList[i]=anchors[counter++];
...
vector<ellipseTrial> User::ellipseHelper(short hv[][anchorSize],int  ah,int  h3)
{
    ...
    for(int i=0;i<anchor;i++)
    {
      for(int j=0;j<anchor;j++)
      {
          ...
          {
            //cout<<"Threanodesd is working"<<endl;
...
string User::addToTAS(int cx, int cy)
{
...
void User :: getTrainInfo(short hv[][anchorSize], short hopLength)
{
    radioRange = hopLength;
    for(int i=0;i<anchor;i++)
    {
        for(int j=0;j<anchor;j++)
        {
    ...
    for(int i=0;i<anchor;i++)
    {
        ...
        for(int j=0;j<anchor;j++)
        {
            if(i!=j){
                int neighborX=xList[j];
                int neighborY=yList[j];
                hv[i][j] = static_cast<int>(ceil(euDis2(curX,curY,neighborX,neighborY)/hopLength));
...
vector<hyperTrial> User::findTrainHyperTrial(short hv[][anchorSize])
{
    ...
    for(int i=1;i<=25;i++)
    {
      for(int j=1;j<=25;j++)
      {
        VF.push_back(async(&User::trainHyperHelper,this,hv,j,i));
      }
    }
...
vector<hyperTrial> User::trainHyperHelper(short hv[][anchorSize],int  ah,int  h3)
{
    vector<hyperTrial> trials;
    ...
    for(int i=0;i<anchor;i++)
    {
      for(int j=0;j<anchor;j++)
      {
        if(i!=j){
          float avgHopDis1= radioRange;
          float avgHopDis2= radioRange;
          float dis=euDis2(xList[i],yList[i],xList[j],yList[j]);
          Point p1={xList[i],yList[i]};
          Point p2={xList[j],yList[j]};
          ...
          if(dis>2*ah*avgHopDis1)
          {
            for(int z=0;z<anchor;z++)
            {
              //cout<<"Thread is working"<<endl;
              Point p3={xList[z],yList[z]};
              float dis2=euDis2(xList[z],yList[z],xList[j],yList[j]);
              float dis3=euDis2(xList[z],yList[z],xList[i],yList[i]);
              if(hv[z][j]!=-1&&hv[z][j]!=0 && hv[z][i]!=-1&&hv[z][i]!=0)
              {
                  ...
                  if(area1-area2>3 && area1-area2 < 400){
                    float yroot[8]={0.0};
                    int startInd=0;
                    for(float& y1:yr1)
                    {
