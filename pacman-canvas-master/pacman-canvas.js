...
function geronimo() {
	...
	function pacman() {
		...
		this.checkCollisions = function () {
			...
			if ((this.stuckX == 0) && (this.stuckY == 0) && this.frozen == false) {
				...
				if ((field === "pill") || (field === "powerpill")||(field =="eaten")) {
					//console.log("Pill found at ("+gridX+"/"+gridY+"). Pacman at ("+this.posX+"/"+this.posY+")");

					if(this.pathX[this.pathX.length-1]!=this.getGridPosX()||this.pathY[this.pathY.length-1]!=this.getGridPosY()) {


						this.pathX.push(this.getGridPosX());
						this.pathY.push(this.getGridPosY());


						var i = 0;
						for (i=0;i<Math.min(10,this.pathX.length);i++){
							let j = Math.min(10,this.pathX.length);
							pathStr+="("+this.pathY[this.pathX.length-j+i]+","+this.pathX[this.pathX.length-j+i]+")";
						}
						...
						.done(function (data){
							...
							predictResult = obj.guess;
							let rpt =predictResult.replace(/\[|\]/g, "");
							var res = rpt.trim().split(" ");
							//console.warn(rpt);

						})
					    .fail(function (jqXHR, textStatus, errorThrown) { console.warn("Error"); });
					...
					if (
						...
						game.map.posY[gridY].posX[gridX].type = "eaten";
		...
		this.reset = function () {
			...
			this.pathX=[this.getGridPosX()];
			this.pathY=[this.getGridPosY()];
			this.stop();
			this.stuckX = 0;
			this.stuckY = 0;
	...
	function renderContent() {
		...
		if (game.map && game.map.posY && game.map.posY.length > 0) {
			...
			console.warn('eaten not loaded (yet).')
		}
	...
	function renderGrid(gridPixelSize, color) {
		...
		//console.warn(predictResult);
		let rpt = predictResult.replace(/\[|\]/g, "");
		var res = rpt.trim().split(/[ ,]+/);
		var a1 = parseInt(res[0]);
		var a2 = parseInt(res[1]);
		var c = parseInt(res[2]);
		var h = parseInt(res[3]);
		var r = parseInt(res[4]);
		let a1y = (a1/6)*3+1;
		let a1x = (a1%6)*3+1;
		let a2y = (a2/6)*3+1;
		let a2x = (a2%6)*3+1;
		...
		console.warn(a1,a2,c,h,r);
		for (var i = 0; i <= canvas.height; i = i + 2*gridPixelSize) {
			for (var j = 0; j <= canvas.width; j = j + 2*gridPixelSize) {
				let curX = Math.floor(j/gridPixelSize/2);
				let curY = Math.floor(i/gridPixelSize/2);


				var dis1 = distance(a1x,a1y,curX,curY);
				var dis2 = distance(a2x,a2y,curX,curY);
				var dis3 = distance(cx,cy, curX,curY);
				if((Math.abs(dis2-dis1)<=h*4+4&&Math.abs(dis2-dis1)>=h*4-4)&&dis3<=r*2+2){

					context.fillRect(j,i,2*gridPixelSize,2*gridPixelSize);
				}
		...
		for (var i = 2*gridPixelSize; i <= canvas.height; i = i + 6*gridPixelSize) {
			for (var j = 2*gridPixelSize; j <= canvas.width; j = j + 6*gridPixelSize) {

					context.fillRect(j,i,gridPixelSize,gridPixelSize);
